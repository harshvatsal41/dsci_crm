...
export async function POST(req) {
    try {
        ...
        if (!agendaId) {
            return NextResponse.json(
                apiResponse({
                    message: "Agenda ID is required for update",
        ...
        const rawData = { // Required for update
        ...
        // Check for time conflicts (excluding current agenda)
        const timeConflict = await Agenda.findOne({
            _id: { $ne: agendaId }, // Exclude current agenda
        ...
        updatedAgenda.title = agendaData.title;
        updatedAgenda.description = agendaData.description;
        updatedAgenda.yeaslyEventId = agendaData.yeaslyEventId;
        updatedAgenda.day = agendaData.day;
        updatedAgenda.date = agendaData.date;
        updatedAgenda.category = agendaData.category;
        updatedAgenda.type = agendaData.type;
        updatedAgenda.startTime = agendaData.startTime;
        updatedAgenda.endTime = agendaData.endTime;
        updatedAgenda.session = agendaData.session;
        updatedAgenda.updatedBy = decodedToken.id;

        await updatedAgenda.save();
        ...
        console.error("PUT /api/agenda error:", error);
